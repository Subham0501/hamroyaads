{{-- Birthday Template 2: Party Time - Fun Banner Layout with Dates --}}
@php
...
<div class="min-h-screen bg-white dark:bg-[#0f172a]" data-theme-color="{{ $color }}" id="template-container" style="--theme-color: {{ $color }};">
    ...
    <!-- Hero Section - Fun Banner -->
    <section id="home" class="relative min-h-screen overflow-hidden pt-20">
        <div class="absolute inset-0 z-0">
            <img id="hero-image-display" data-image="hero" src="{{ $categoryImages['hero'] ?? 'https://images.unsplash.com/photo-1530103862676-de8c9debad1d?w=1600&q=80' }}" alt="Hero" class="w-full h-full object-cover opacity-60">
            <div class="absolute inset-0 bg-gradient-to-br" style="background: linear-gradient(135deg, {{ $color }}80, transparent);"></div>
        </div>
        <div class="relative z-10 min-h-screen flex items-center px-12 py-32">
            <div class="max-w-6xl mx-auto w-full">
                <div class="bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-xl rounded-[2rem] shadow-2xl p-16 border-4 animate-fade-in" style="border-color: {{ $color }};">
                    <div class="text-center space-y-10">
                        <div class="text-8xl mb-6 animate-float">🎂</div>
                        <div class="mb-6">
                            <span class="text-sm font-semibold" style="color: {{ $color }}80;">{{ $currentDate }}</span>
            </div>
                        <h1 id="preview-heading" class="text-7xl md:text-9xl font-black leading-[0.9] mb-6" style="color: {{ $color }};">
                            {{ $templateData['defaults']['heading'] ?? 'It\'s Your Day!' }}
                        </h1>
                        <p class="text-2xl md:text-3xl font-bold uppercase tracking-wide mb-8" style="color: {{ $color }};">
                            {{ $templateData['defaults']['subheading'] ?? 'Let\'s Celebrate' }}
                        </p>
                        <p id="preview-message" class="text-xl md:text-2xl leading-relaxed" style="color: {{ $color }}90;">
                            {{ Str::limit($templateData['defaults']['message'] ?? 'Your heartfelt message here', 85) }}
                        </p>
