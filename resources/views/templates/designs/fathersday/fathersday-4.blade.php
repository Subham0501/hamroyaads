{{-- Father's Day Template 4: Modern Tribute - Asymmetric Layout with Dates --}}
@php
...
<div class="min-h-screen bg-white dark:bg-[#0f172a]" data-theme-color="{{ $color }}" id="template-container" style="--theme-color: {{ $color }};">
    <!-- Navigation -->
    <nav class="fixed top-0 left-0 right-0 z-50 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-xl border-b-2 transition-all duration-500" style="border-color: {{ $color }}40;">
    ...
    <section id="home" class="relative min-h-screen flex items-center px-12 py-32 overflow-hidden pt-20">
        <div class="max-w-7xl mx-auto w-full">
            <div class="grid lg:grid-cols-2 gap-16 items-center">
                <div class="relative order-2 lg:order-1 animate-scale-in">
                    <div class="relative rounded-[2rem] overflow-hidden shadow-2xl transform rotate-3 border-6 border-white">
                        <img id="hero-image-display" data-image="hero" src="{{ $categoryImages['hero'] ?? 'https://images.unsplash.com/photo-1511988617509-a57c8a288659?w=1600&q=80' }}" alt="Hero" class="w-full h-[600px] object-cover">
                    </div>
                </div>
                <div class="space-y-10 order-1 lg:order-2 animate-slide-up">
                    <div class="inline-flex items-center gap-4 px-6 py-3 rounded-full backdrop-blur-md border-2" style="background-color: {{ $color }}30; border-color: {{ $color }};">
                        <svg class="w-8 h-8" style="color: {{ $color }};" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                        </svg>
                        <p id="preview-subheading" class="text-xl font-bold uppercase tracking-wide" style="color: {{ $color }};">{{ $templateData['defaults']['subheading'] ?? 'Celebrating You' }}</p>
                    </div>
                    <div class="mb-4">
                        <span class="text-sm font-semibold" style="color: {{ $color }}80;">{{ $currentDate }}</span>
                    </div>
                    <h1 id="preview-heading" class="text-6xl md:text-8xl font-black leading-[0.9]" style="color: {{ $color }};">
                        {{ $templateData['defaults']['heading'] ?? 'To The Best Dad' }}
                    </h1>
                    <p id="preview-message" class="text-xl md:text-2xl leading-relaxed text-gray-700 dark:text-[#cbd5e1]">
                        {{ Str::limit($templateData['defaults']['message'] ?? 'Your heartfelt message here', 75) }}
                    </p>
    ...
    <section id="story" class="bg-gradient-to-br {{ $bg }} py-32 px-8">
        <div class="max-w-6xl mx-auto">
            <div class="grid md:grid-cols-2 gap-8">
                <div class="bg-white dark:bg-[#1e293b] rounded-3xl p-10 shadow-xl border-2 animate-fade-in" style="border-color: {{ $color }};">
                    <div class="mb-4">
    ...
    <section class="bg-white dark:bg-[#0f172a] py-32 px-8 relative overflow-hidden">
        <div class="max-w-6xl mx-auto">
            <div class="relative">
    ...
    <section class="bg-gradient-to-br {{ $bg }} py-32 px-8">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mt-12 text-center">
                <p id="section2-content" class="text-xl md:text-2xl leading-relaxed max-w-3xl mx-auto" style="color: {{ $color }}90;">
