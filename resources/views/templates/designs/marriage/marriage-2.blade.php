{{-- Marriage Template 2: Classic Romance - Magazine Style Layout with Dates --}}
@php
...
<div class="min-h-screen bg-white dark:bg-[#0f172a]" data-theme-color="{{ $color }}" id="template-container" style="--theme-color: {{ $color }};">
    ...
    <!-- Hero Section - Magazine Header Style -->
    <section id="home" class="relative border-b-8 overflow-hidden pt-20" style="border-color: {{ $color }};">
        <div class="absolute inset-0 z-0">
            <img id="hero-image-display" data-image="hero" src="{{ $categoryImages['hero'] ?? 'https://images.unsplash.com/photo-1519741497674-611481863552?w=1600&q=80' }}" alt="Hero" class="w-full h-[85vh] object-cover">
            <div class="absolute inset-0 bg-gradient-to-r from-black/85 via-black/60 to-black/40"></div>
        </div>
        <div class="relative z-10 max-w-7xl mx-auto px-12 py-40">
            <div class="max-w-4xl">
                <div class="mb-6 animate-fade-in">
                    <span class="text-sm font-bold uppercase tracking-widest text-white/90">{{ $currentDate }}</span>
                </div>
                <h1 id="preview-heading" class="text-7xl md:text-9xl lg:text-[11rem] font-black text-white leading-[0.85] mb-8 animate-slide-up">
                    {{ $templateData['defaults']['heading'] ?? 'Forever & Always' }}
                </h1>
                <div class="inline-block px-6 py-3 rounded-full mb-6 backdrop-blur-md animate-scale-in" style="background-color: {{ $color }}40;">
                    <p id="preview-subheading" class="text-lg font-bold uppercase tracking-wider text-white">{{ $templateData['defaults']['subheading'] ?? 'Our Journey Begins' }}</p>
                </div>
            </div>
        </div>
    </section>
    
    <!-- Message Section - Sidebar Layout -->
    <section id="story" class="bg-white dark:bg-[#0f172a] py-32 px-12">
        <div class="max-w-7xl mx-auto">
            <div class="grid lg:grid-cols-3 gap-16">
                <div class="lg:col-span-1">
                    <div class="sticky top-28">
                        <div class="w-full aspect-square rounded-3xl flex items-center justify-center shadow-2xl transform rotate-3 border-8 border-white dark:border-gray-800 animate-scale-in" style="background-color: {{ $color }};">
                ...
                <div class="lg:col-span-2 space-y-8 animate-slide-up">
                    <p id="preview-message" class="text-3xl md:text-4xl text-gray-800 dark:text-[#e2e8f0] leading-relaxed font-light">
                        {{ $templateData['defaults']['message'] ?? 'Your heartfelt message here' }}
                    </p>
                    <div class="pt-8 border-t-2" style="border-color: {{ $color }};">
    ...
    <!-- Section 1 - Alternating Cards -->
    <section class="bg-gradient-to-br {{ $bg }} py-32 px-8 relative overflow-hidden">
        <div class="max-w-6xl mx-auto">
            <div class="bg-white dark:bg-[#1e293b] rounded-[2rem] shadow-2xl p-12 border-4 animate-fade-in" style="border-color: {{ $color }};">
    ...
    <section class="bg-white dark:bg-[#0f172a] py-32 px-8">
        <div class="max-w-7xl mx-auto">
            <div class="grid md:grid-cols-2 gap-16 items-center">
                <div class="relative order-2 md:order-1 animate-scale-in">
                    <div class="aspect-square rounded-3xl overflow-hidden shadow-2xl border-4" style="border-color: {{ $color }};">
                ...
                <div class="space-y-6 order-1 md:order-2 animate-slide-up">
                    <div class="inline-flex items-center gap-3">
                        <div class="w-12 h-1 rounded-full" style="background-color: {{ $color }};"></div>
                        <span class="text-sm font-bold uppercase tracking-wider" style="color: {{ $color }};">Chapter Two</span>
                    </div>
                    <h2 id="section2-title" class="text-4xl md:text-5xl font-black mb-4" style="color: {{ $color }};">{{ $templateData['defaults']['section2_title'] ?? 'Our First Day' }}</h2>
                    <p class="text-sm font-semibold mb-4" style="color: {{ $color }}80;">{{ $currentDate }}</p>
    ...
    <!-- Section 3 - Masonry Style -->
    <section class="bg-gradient-to-br {{ $bg }} py-32 px-8 relative overflow-hidden">
        <div class="max-w-7xl mx-auto">
            <div class="text-center mb-16 animate-fade-in">
                <h2 id="section3-title" class="text-5xl md:text-6xl font-black mb-4" style="color: {{ $color }};">{{ $templateData['defaults']['section3_title'] ?? 'Our Journey' }}</h2>
                <p class="text-sm font-semibold mb-4" style="color: {{ $color }}80;">{{ $currentDate }}</p>
            </div>
            <div class="grid md:grid-cols-3 gap-6">
                @for($i = 1; $i <= 3; $i++)
                <div class="aspect-[4/5] rounded-2xl overflow-hidden shadow-xl border-2 animate-scale-in" style="border-color: {{ $color }}40; animation-delay: {{ $i * 0.15 }}s;">
                    <div class="w-full h-full bg-gradient-to-br flex items-center justify-center" style="background: linear-gradient(135deg, {{ $color }}30, {{ $color }}10);">
