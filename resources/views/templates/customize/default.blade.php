                ...
                <div class="bg-white dark:bg-[#1e293b] rounded-2xl shadow-xl p-8 border border-gray-100 dark:border-[#334155]">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="border-2 border-gray-200 dark:border-[#334155] rounded-xl p-6">
                            ...
                            <div id="section2-upload-area" class="border-2 border-dashed border-gray-300 dark:border-[#334155] rounded-lg p-4 text-center hover:border-blue-500 transition-colors cursor-pointer" onclick="highlightUploadArea('section2-upload-area'); document.getElementById('section2-image')?.click()">
                                <input type="file" id="section2-image" accept="image/jpeg,image/jpg,image/png,image/gif,image/webp" class="hidden" data-image-type="section2">
...
function setupImageUpload(inputId, imageType, previewId, previewImgId) {
    ...
    if (input) {
        ...
        if (inputId.includes('section2-image') || inputId.includes('section3-image') || inputId === 'hero-image') {
            ...
            } else if (inputId === 'section2-image') {
                uploadDiv = document.getElementById('section2-upload-area');
                areaId = 'section2-upload-area';
...
function updatePreviewImage(imageType, imageSrc) {
    ...
    if (imageType === 'hero') {
        ...
        // Find all section 2 image instances - try multiple selectors
        let imgs = previewContainer.querySelectorAll('#section2-image-display');
        let placeholders = previewContainer.querySelectorAll('#section2-placeholder');
        
        // If not found, try without the # (might be in iframe or different structure)
        if (imgs.length === 0) {
            imgs = previewContainer.querySelectorAll('[id*="section2-image"][id*="display"], [id^="section2-image"]:not([id*="preview"])');
        }
        if (placeholders.length === 0) {
            placeholders = previewContainer.querySelectorAll('[id*="section2-placeholder"], [id^="section2-placeholder"]');
        }
        
        console.log('Updating section2 image:', { imageType, imgCount: imgs.length, placeholderCount: placeholders.length, imageSrc: !!imageSrc });
        ...
        if (imgs.length > 0) {
            ...
            imgs.forEach((img, index) => {
                if (imageSrc) {
                    ...
                    console.log(`Section2 image displayed in template ${index + 1}`, img);
                } else {
                    img.classList.add('hidden');
                    img.style.display = 'none';
                }
        ...
        if (placeholders.length > 0) {
            placeholders.forEach((placeholder, index) => {
                if (imageSrc) {
                    ...
                    placeholder.style.display = 'block';
                    placeholder.classList.remove('hidden');
                    console.log(`Section2 placeholder shown in template ${index + 1}`);
                }
            });
        }
        
        if (imgs.length === 0 && placeholders.length === 0) {
            console.warn('Section2 image elements not found. Searching in:', previewContainer);
            // Debug: log all elements with section2 in ID
            const allSection2 = previewContainer.querySelectorAll('[id*="section2"]');
            console.log('All section2 elements:', allSection2.length, Array.from(allSection2).map(el => el.id));
        }
    } else if (imageType.startsWith('section3-')) {
        const num = imageType.split('-')[1];
